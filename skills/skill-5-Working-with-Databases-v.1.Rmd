...
head(needles)
```
Example of a simple SQL query to retrive only certain observations from the ```needle_events``` table:

```{r}
dbGetQuery(cn, "SELECT * FROM needle_events where NEEDLE_QUANTITY>1")
```
...
```{r}
dbGetQuery(cn, "SELECT * FROM needle_events inner join locations_geo on needle_events.LOCATION_RECORDED=locations_geo.LOCATION_RECORDED")
